...
We have adopted a [Code of Conduct](https://github.com/infini-design/infini-design/blob/master/CODE_OF_CONDUCT.md) that we expect project participants to adhere to. Please read the full text so that you can understand what actions will and will not be tolerated.

## Open Development

All work on Ant Design happens directly on [GitHub](https://github.com/infini-design). Both core team members and external contributors send pull requests which go through the same review process.
...
We are using [GitHub Issues](https://github.com/infini-design/infini-design/issues) for bug tracing. The best way to get your bug fixed is using our [issue helper](http://new-issue.ant.design) and provide a reproduction with this [template](https://u.ant.design/codesandbox-repro).
...
If you want to help on new API, please ref [API Naming Rules](https://github.com/infini-design/infini-design/wiki/API-Naming-rules) to name it.
...
Since antd's components are based on react-component, sometimes you may need to send pull request to the corresponding react-component repository. If it's a bugfix pull request, after it's merged, the core team will release a patch release for that component as soon as possible, then you only need to do is reinstalling antd in your project to get the latest patch release. If it's a feature pull request, after it's merged, the core team will release a minor release, then you need raise another pull request to [Ant Design](https://github.com/infini-design/infini-design/) to update dependencies, document and TypeScript interfaces (if needed).
