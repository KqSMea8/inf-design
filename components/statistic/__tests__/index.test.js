...
describe('Statistic', () => {
  ...
  it('customize formatter', () => {
    ...
    expect(wrapper.find('.infini-statistic-content-value').text()).toEqual('93');
  });

  it('groupSeparator', () => {
    const wrapper = mount(<Statistic value={1128} groupSeparator="__TEST__" />);
    expect(wrapper.find('.infini-statistic-content-value').text()).toEqual('1__TEST__128');
  });

  it('not a number', () => {
    const wrapper = mount(<Statistic value="bamboo" />);
    expect(wrapper.find('.infini-statistic-content-value').text()).toEqual('bamboo');
  });
  ...
  describe('Countdown', () => {
    it('render correctly', () => {
      ...
      [
        ...
        expect(wrapper.find('.infini-statistic-content-value').text()).toEqual(value);
      });
    });
