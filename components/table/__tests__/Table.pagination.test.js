...
describe('Table.pagination', () => {
  ...
  it('should not show pager if pagination.hideOnSinglePage is true and only 1 page', () => {
    ...
    expect(wrapper.find('.infini-pagination')).toHaveLength(1);
    ...
    expect(wrapper.find('.infini-pagination')).toHaveLength(0);
  ...
  it('should use pageSize when defaultPageSize and pageSize are both specified', () => {
    ...
    expect(wrapper.find('.infini-pagination-item')).toHaveLength(2);
  });
  ...
  // https://github.com/infini-design/infini-design/issues/4532
  ...
  it('should have pager when change pagination from false to undefined', () => {
    ...
    expect(wrapper.find('.infini-pagination')).toHaveLength(1);
    expect(wrapper.find('.infini-pagination-item-active')).toHaveLength(1);
  });
  ...
  it('should display pagination as prop pagination change between true and false', () => {
    ...
    expect(wrapper.find('.infini-pagination')).toHaveLength(1);
    expect(wrapper.find('.infini-pagination-item')).toHaveLength(2);
    wrapper.find('.infini-pagination-item-2').simulate('click');
    expect(renderedNames(wrapper)).toEqual(['Tom', 'Jerry']);
    wrapper.setProps({ pagination: false });
    expect(wrapper.find('.infini-pagination')).toHaveLength(0);
    wrapper.setProps({ pagination: true });
    expect(wrapper.find('.infini-pagination')).toHaveLength(1);
    expect(wrapper.find('.infini-pagination-item')).toHaveLength(1); // pageSize will be 10
    expect(renderedNames(wrapper)).toEqual(['Jack', 'Lucy', 'Tom', 'Jerry']);
  });

  // https://github.com/infini-design/infini-design/issues/5259
  it('change to correct page when data source changes', () => {
    const wrapper = mount(createTable({ pagination: { pageSize: 1 } }));
    wrapper.find('.infini-pagination-item-3').simulate('click');
    wrapper.setProps({ dataSource: [data[0]] });
    expect(wrapper.find('.infini-pagination-item-1').hasClass('infini-pagination-item-active')).toBe(
      true,
    );
  });
  ...
  it('specify the position of pagination', () => {
    ...
    expect(wrapper.find('.infini-spin-container').children()).toHaveLength(2);
    expect(
      wrapper
        .find('.infini-spin-container')
        .childAt(0)
        .find('.infini-pagination'),
    ).toHaveLength(1);
    ...
    expect(wrapper.find('.infini-spin-container').children()).toHaveLength(3);
    expect(
      wrapper
        .find('.infini-spin-container')
        .childAt(0)
  ...
  it('ajax render should keep display by the dataSource', () => {
    ...
    expect(wrapper.find('.infini-table-tbody tr.infini-table-row')).toHaveLength(data.length);

    wrapper.find('.infini-pagination .infini-pagination-item-2').simulate('click');
