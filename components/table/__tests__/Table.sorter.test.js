...
describe('Table.sorter', () => {
  ...
  it('sort records', () => {
    ...
    // ascend
    wrapper.find('.infini-table-column-sorters').simulate('click');
    expect(renderedNames(wrapper)).toEqual(['Jack', 'Jerry', 'Lucy', 'Tom']);
  ...
  // https://github.com/infini-design/infini-design/issues/11246#issuecomment-405009167
  ...
  // https://github.com/infini-design/infini-design/pull/12264#discussion_r218053034
  it('should sort from begining state when toggle from different columns', () => {
    ...
    const wrapper = mount(<Table columns={columns} dataSource={testData} />);
    const nameColumn = wrapper.find('.infini-table-column-sorters').at(0);
    const ageColumn = wrapper.find('.infini-table-column-sorters').at(1);
    // sort name
    nameColumn.simulate('click');
    expect(
      nameColumn
        .find('.infini-table-column-sorter-up')
        .at(0)
  ...
  // https://github.com/infini-design/infini-design/issues/12571
  it('should toggle sort state when columns are put in render', () => {
    ...
    expect(
      nameColumn
        .find('.infini-table-column-sorter-down')
        .at(0)
        .getDOMNode().className,
    ).toContain(' off');
    // sort name
  ...
  // https://github.com/infini-design/infini-design/issues/12737
  ...
  // https://github.com/infini-design/infini-design/issues/12870
