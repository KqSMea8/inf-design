...
describe('Avatar Render', () => {
  ...
  beforeAll(() => {
    ...
    Object.defineProperty(HTMLElement.prototype, 'offsetWidth', {
      get() {
        if (this.className === 'infini-avatar-string') {
          return 100;
        }
        return 80;
      },
  ...
  it('Render long string correctly', () => {
    ...
    const children = wrapper.find('.infini-avatar-string');
  ...
  it('should render fallback string correctly', () => {
    ...
    expect(div.querySelector('.infini-avatar-string').style.transform).toContain('scale(0.5)');
  ...
  it('should show image on success after a failure state', () => {
    ...
    expect(wrapper.find('.infini-avatar-string').length).toBe(1);
    ...
    expect(wrapper.find(Avatar).instance().state.isImgExist).toBe(true);
    expect(wrapper.find('.infini-avatar-image').length).toBe(1);
