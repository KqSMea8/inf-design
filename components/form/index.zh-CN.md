...
| setFieldsValue | 设置一组输入控件的值（注意：不要在 `componentWillReceiveProps` 内使用，否则会导致死循环，[原因](https://github.com/infini-design/infini-design/issues/2985)） | ({ \[fieldName\]&#x3A; value }) => void |
...
| validator | 自定义校验（注意，[callback 必须被调用](https://github.com/infini-design/infini-design/issues/5155)） | function(rule, value, callback) | - |
