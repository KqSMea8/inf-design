...
describe('Transfer', () => {
  ...
  it('should check all item when click on check all', () => {
    ...
    const wrapper = mount(<Transfer {...listCommonProps} onSelectChange={handleSelectChange} />);
    wrapper
      .find('.infini-transfer-list-header input[type="checkbox"]')
  ...
  const headerText = wrapper =>
    wrapper
      .find(TransferList)
      .at(0)
      .find('.infini-transfer-list-header-selected > span')
      .at(0)
      .first()
      .text()
      .trim();
  ...
  it('should display the correct locale', () => {
    ...
    expect(
      wrapper
        .find(TransferList)
        .at(0)
        .find('.infini-transfer-list-search')
        .at(0)
        .prop('placeholder'),
    ).toEqual('Search');
    ...
    expect(
      wrapper
        .find(TransferList)
        .at(0)
        .find('.infini-transfer-list-body-not-found')
        .at(0)
        .text(),
    ).toEqual('Nothing');
  });
  ...
  it('should add custom styles when their props are provided', () => {
    ...
    const wrapper = component.find('.infini-transfer');
    const listSource = component.find('.infini-transfer-list').first();
    const listTarget = component.find('.infini-transfer-list').last();
    const operation = component.find('.infini-transfer-operation').first();
