...
describe('message', () => {
  ...
  it('should be able to config top', () => {
    ...
    expect(document.querySelectorAll('.infini-message')[0].style.top).toBe('100px');
  });
  ...
  it('should be able to config maxCount', () => {
    ...
    message.info('last');
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(5);
    expect(document.querySelectorAll('.infini-message-notice')[4].textContent).toBe('last');
    jest.runAllTimers();
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(0);
  });

  it('should be able to hide manually', () => {
    const hide1 = message.info('whatever', 0);
    const hide2 = message.info('whatever', 0);
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(2);
    hide1();
    jest.runAllTimers();
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(1);
    hide2();
    jest.runAllTimers();
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(0);
  });
  ...
  it('should be able to destroy globally', () => {
    ...
    message.info('whatever', 0);
    expect(document.querySelectorAll('.infini-message').length).toBe(1);
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(2);
    message.destroy();
    expect(document.querySelectorAll('.infini-message').length).toBe(0);
    expect(document.querySelectorAll('.infini-message-notice').length).toBe(0);
  });
  ...
  // https://github.com/infini-design/infini-design/issues/8201
  ...
  it('should have no icon', () => {
    ...
    expect(document.querySelectorAll('.infini-message-notice .anticon').length).toBe(0);
  });
