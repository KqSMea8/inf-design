...
describe('Cascader', () => {
  ...
  it('can be selected', () => {
    ...
    popupWrapper
      .find('.infini-cascader-menu')
      .at(0)
      .find('.infini-cascader-menu-item')
      .at(0)
      .simulate('click');
    expect(
      render(
  ...
  it('should support to clear selection', () => {
    const wrapper = mount(<Cascader options={options} defaultValue={['zhejiang', 'hangzhou']} />);
    expect(wrapper.find('.infini-cascader-picker-label').text()).toBe('Zhejiang / Hangzhou');
    wrapper
      .find('.infini-cascader-picker-clear')
      .at(0)
      .simulate('click');
    expect(wrapper.find('.infini-cascader-picker-label').text()).toBe('');
  });
  ...
  it('should change filtered item when options are changed', () => {
    ...
    expect(wrapper.find('.infini-cascader-menu-item').length).toBe(2);
    wrapper.setProps({ options: [options[0]] });
    expect(wrapper.find('.infini-cascader-menu-item').length).toBe(1);
  });
  ...
  it('can use fieldNames', () => {
    ...
    expect(
      wrapper
        .find('.infini-cascader-picker-label')
        .text()
        .split('/').length,
    ).toBe(3);
  });

  // https://github.com/infini-design/infini-design/issues/12970
