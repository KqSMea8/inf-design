...
describe('Badge', () => {
  ...
  it('badge dot not scaling count > 9', () => {
    ...
    expect(badge.find('.infini-card-multiple-words').length).toBe(0);
  });
  ...
  it('badge dot not showing count == 0', () => {
    ...
    expect(badge.find('.infini-badge-dot').length).toBe(0);
  });
  ...
  it('should have an overriden title attribute', () => {
    ...
    expect(
      badge
        .find('.infini-scroll-number')
        .getDOMNode()
        .attributes.getNamedItem('title').value,
    ).toEqual('Custom title');
  });

  // https://github.com/infini-design/infini-design/issues/10626
  ...
  // https://github.com/infini-design/infini-design/issues/13694
  ...
  // https://github.com/infini-design/infini-design/issues/15349
  it('should color style  works on Badge', () => {
    const wrapper = mount(<Badge style={{ color: 'red' }} status="success" text="Success" />);
    expect(wrapper.find('.infini-badge-status-text').props().style.color).toBe('red');
  });

  // https://github.com/infini-design/infini-design/issues/15799
