...
describe('DatePicker', () => {
  ...
  // Fix https://github.com/infini-design/infini-design/issues/8885
  it('control value after panel closed', () => {
    ...
    expect(wrapper.find('.infini-calendar-input').getDOMNode().value).toBe('2016-11-13 12:12:12');
    selectDate(wrapper, moment('2016-11-14'));
    expect(wrapper.find('.infini-calendar-input').getDOMNode().value).toBe('2016-11-14 12:12:12');
  });
  ...
  it('sets data attributes on input', () => {
    ...
    const input = wrapper.find('.infini-calendar-picker-input').getDOMNode();
  ...
  it('changes year/month when under control', () => {
    ...
    openPanel(wrapper);
    expect(wrapper.find('.infini-calendar-my-select').text()).toBe('Jul2018');
    wrapper.find('.infini-calendar-prev-year-btn').simulate('click');
    wrapper.find('.infini-calendar-prev-month-btn').simulate('click');
    expect(wrapper.find('.infini-calendar-my-select').text()).toBe('Jun2017');
  });
  ...
  it('extra footer works', () => {
    ...
    wrapper
      .find('.infini-calendar-month-select')
      .hostNodes()
      .simulate('click');
    extraNode = wrapper.find('.infini-calendar-month-panel .extra-node');
    expect(extraNode.length).toBe(1);
    expect(extraNode.text()).toBe('month');

    wrapper
      .find('.infini-calendar-year-select')
      .hostNodes()
      .simulate('click');
    extraNode = wrapper.find('.infini-calendar-year-panel .extra-node');
    expect(extraNode.length).toBe(1);
    expect(extraNode.text()).toBe('year');

    wrapper
      .find('.infini-calendar-year-panel-decade-select')
      .hostNodes()
      .simulate('click');
    extraNode = wrapper.find('.infini-calendar-decade-panel .extra-node');
    expect(extraNode.length).toBe(1);
  ...
  it('supports multiple formats', () => {
    ...
    openPanel(wrapper);
    wrapper.find('.infini-calendar-input').simulate('change', { target: { value: '02/07/18' } });
    expect(wrapper.find('.infini-calendar-picker-input').getDOMNode().value).toBe('02/07/2018');
  });
