...
describe('RangePicker', () => {
  ...
  it('switch to corresponding month panel when click presetted ranges', () => {
    ...
    rangeCalendarWrapper.find('.infini-calendar-range-quick-selector Tag').simulate('click');
    expect(
      render(
        wrapper
          .find('Trigger')
  ...
  it('highlight range when hover presetted range', () => {
    ...
    rangeCalendarWrapper.find('.infini-calendar-range-quick-selector Tag').simulate('mouseEnter');
    ...
    expect(rangeCalendarWrapper.find('.infini-calendar-selected-day').length).toBe(2);
  });
  ...
  it('should trigger onCalendarChange when change value', () => {
    ...
    rangeCalendarWrapper
      .find('.infini-calendar-cell')
      .at(15)
      .simulate('click');
    expect(onCalendarChangeFn).toHaveBeenCalled();
  });

  // issue: https://github.com/infini-design/infini-design/issues/5872
  ...
  // issue: https://github.com/infini-design/infini-design/issues/7077
  it('should not throw error when select after clear', () => {
    ...
    wrapper.update();
    wrapper
      .find('.infini-calendar-picker-clear')
      .hostNodes()
      .simulate('click');
    wrapper.find('.infini-calendar-picker-input').simulate('click');
    rangeCalendarWrapper = mount(
      wrapper
        .find('Trigger')
        .instance()
  ...
  it('clear hover value after panel close', () => {
    ...
    expect(
      wrapper
        .find('.infini-calendar-cell')
        .at(23)
        .hasClass('infini-calendar-in-range-cell'),
    ).toBe(true);
  });
  ...
  describe('preset range', () => {
    it('static range', () => {
      ...
      wrapper.find('.infini-calendar-picker-input').simulate('click');
      wrapper.find('.infini-calendar-range-quick-selector Tag').simulate('click');
      expect(
        wrapper
          .find('.infini-calendar-range-picker-input')
          .first()
          .getDOMNode().value,
  ...
  // https://github.com/infini-design/infini-design/issues/6999
  it('input date manually', () => {
    const wrapper = mount(<RangePicker open />);
    const dateString = '2008-12-31';
    const input = wrapper.find('.infini-calendar-input').first();
  ...
  // https://github.com/infini-design/infini-design/issues/9267
  ...
  it('changes year/month when under control', () => {
    ...
    expect(
      wrapper
        .find('.infini-calendar-my-select')
        .first()
        .text(),
    ).toBe('Jul2018');
    wrapper
      .find('.infini-calendar-prev-year-btn')
      .first()
      .simulate('click');
    wrapper
      .find('.infini-calendar-prev-month-btn')
      .first()
      .simulate('click');
    expect(
      wrapper
  ...
  // https://github.com/infini-design/infini-design/issues/11631
  ...
  // https://github.com/infini-design/infini-design/issues/13302
  describe('in "month" mode, when the left and right panels select the same month', () => {
    ...
    it('the cell status is correct', () => {
      ...
      wrapper.find('.infini-calendar-picker-input').simulate('click');
      wrapper
        .find('.infini-calendar-range-left .infini-calendar-month-panel-cell')
        .at(3)
        .simulate('click');
      wrapper
        .find('.infini-calendar-range-right .infini-calendar-month-panel-cell')
        .at(3)
        .simulate('click');
      expect(
        wrapper
          .find('.infini-calendar-range-left .infini-calendar-month-panel-cell')
          .at(3)
          .hasClass('infini-calendar-month-panel-selected-cell'),
      ).toBe(true);
      expect(
        wrapper
          .find('.infini-calendar-range-left .infini-calendar-month-panel-cell')
      ...
      expect(
        wrapper
          .find('.infini-calendar-range-left .infini-calendar-month-panel-cell')
          .at(2)
          .hasClass('infini-calendar-month-panel-cell-disabled'),
      ).toBe(false);
      expect(
        wrapper
          .find('.infini-calendar-range-left .infini-calendar-month-panel-cell')
