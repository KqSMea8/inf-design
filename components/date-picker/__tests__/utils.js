...
export function selectDate(wrapper, date, index) {
  ...
  if (index !== undefined) {
    calendar = wrapper.find('.infini-calendar-range-part').at(index);
  }
...
export function hasSelected(wrapper, date) {
  return wrapper
    ...
    .hasClass('infini-calendar-selected-day');
}
...
export function nextYear(wrapper) {
  wrapper.find('.infini-calendar-next-year-btn').simulate('click');
}

export function nextMonth(wrapper) {
  wrapper.find('.infini-calendar-next-month-btn').simulate('click');
}
