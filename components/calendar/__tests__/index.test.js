...
describe('Calendar', () => {
  ...
  it('dates other than in valid range should be disabled', () => {
    ...
    const elem = wrapper
      .find('[title="February 20, 2018"]')
      .hasClass('infini-fullcalendar-disabled-cell');
  ...
  it('months other than in valid range should be disabled', () => {
    ...
    expect(
      wrapper
        .find('[title="Jan"]')
        .at(0)
        .hasClass('infini-fullcalendar-month-panel-cell-disabled'),
    ).toBe(true);
    expect(
      wrapper
        .find('[title="Feb"]')
  ...
  it('months other than in valid range should not be shown in header', () => {
    ...
    wrapper
      .find('.infini-fullcalendar-year-select')
      .hostNodes()
      .simulate('click');
    wrapper
      .find('.infini-select-dropdown-menu-item')
      .first()
      .simulate('click');
    wrapper
      .find('.infini-fullcalendar-month-select')
      .hostNodes()
      .simulate('click');
    // 2 years and 11 months
    expect(wrapper.find('.infini-select-dropdown-menu-item').length).toBe(13);
  });
  ...
  it('switch should work correctly without prop mode', async () => {
    ...
    expect(wrapper.find('.infini-fullcalendar-table').length).toBe(1);
    expect(wrapper.find('.infini-fullcalendar-month-panel-table').length).toBe(0);
    wrapper.find('.infini-radio-button-input[value="year"]').simulate('change');
    expect(wrapper.find('.infini-fullcalendar-table').length).toBe(0);
    expect(wrapper.find('.infini-fullcalendar-month-panel-table').length).toBe(1);
    expect(onPanelChange).toHaveBeenCalled();
