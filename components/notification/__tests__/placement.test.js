...
describe('Notification.placement', () => {
  ...
  it('change notification placement by `open` method', () => {
    ...
    style = getStyle($$('.infini-notification-topLeft')[0]);
    ...
    expect($$('.infini-notification-topLeft').length).toBe(1);

    // topRight
    open({
      placement: 'topRight',
    });
    style = getStyle($$('.infini-notification-topRight')[0]);
    expect(style.top).toBe(defaultTop);
    ...
    expect($$('.infini-notification-topRight').length).toBe(1);

    // bottomRight
    open({
      placement: 'bottomRight',
    });
    style = getStyle($$('.infini-notification-bottomRight')[0]);
    ...
    expect($$('.infini-notification-bottomRight').length).toBe(1);

    // bottomLeft
    open({
      placement: 'bottomLeft',
    });
    style = getStyle($$('.infini-notification-bottomLeft')[0]);
    expect(style.top).toBe('');
    expect(style.left).toBe('0px');
    expect(style.bottom).toBe(defaultBottom);
    ...
    expect($$('.infini-notification-bottomLeft').length).toBe(1);
  });
  ...
  it('change notification mountNode by `config` method', () => {
    ...
    expect($container.querySelector('.infini-notification')).not.toBe(null);
    $container.remove();
  });
});
