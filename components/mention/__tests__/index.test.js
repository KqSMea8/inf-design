...
describe('Mention', () => {
  ...
  it('change suggestions', () => {
    ...
    jest.runAllTimers();
    expect(container.getDOMNode().querySelectorAll('.infini-mention-dropdown-item').length).toBe(2);
    expect(container.getDOMNode().querySelectorAll('.infini-mention-dropdown-item')[0].innerHTML).toBe(
      'afc163',
    );
    wrapper.setProps({ suggestions: ['yesmeck', 'yiminghe', 'lucy'] });
    jest.runAllTimers();
    expect(container.getDOMNode().querySelectorAll('.infini-mention-dropdown-item').length).toBe(3);
    expect(container.getDOMNode().querySelectorAll('.infini-mention-dropdown-item')[0].innerHTML).toBe(
      'yesmeck',
    );
  });
  ...
  it('select item', () => {
    ...
    // enzyme cannot find .infini-mention-dropdown-item in react 15
    // I don't know why
    if (process.env.REACT === '15') {
      return;
    }
    wrapper
      .find('.infini-mention-dropdown-item')
      .at(0)
      .simulate('mouseDown');
    wrapper
      .find('.infini-mention-dropdown-item')
  ...
  it('defaultSuggestion filter', () => {
    ...
    const items = wrapper.find('div.infini-mention-dropdown-item');
