...
describe('Modal.confirm triggers callbacks correctly', () => {
  ...
  if (process.env.REACT !== '15') {
    it('shows animation when close', () => {
      ...
      open();
      $$('.infini-btn')[0].click();
      expect($$('.infini-modal-confirm')).toHaveLength(1);
      jest.runAllTimers();
      expect($$('.infini-modal-confirm')).toHaveLength(0);
      jest.useRealTimers();
    });
  }
  ...
  it('ok only', () => {
    open({ okCancel: false });
    expect($$('.infini-btn')).toHaveLength(1);
    expect($$('.infini-btn')[0].innerHTML).toContain('OK');
  });

  it('allows extra props on buttons', () => {
    open({ okButtonProps: { disabled: true }, cancelButtonProps: { 'data-test': 'baz' } });
    expect($$('.infini-btn')).toHaveLength(2);
    expect($$('.infini-btn')[0].attributes['data-test'].value).toBe('baz');
    expect($$('.infini-btn')[1].disabled).toBe(true);
  });
  ...
  it('should close modals when click confirm button', () => {
    ...
    ['info', 'success', 'warning', 'error'].forEach(type => {
      ...
      expect($$(`.infini-modal-confirm-${type}`)).toHaveLength(1);
      $$('.infini-btn')[0].click();
      jest.runAllTimers();
      expect($$(`.infini-modal-confirm-${type}`)).toHaveLength(0);
    });
    jest.useRealTimers();
  });
  ...
  it('could be update', () => {
    ...
    ['info', 'success', 'warning', 'error'].forEach(type => {
      ...
      expect($$(`.infini-modal-confirm-${type}`)).toHaveLength(1);
      expect($$('.infini-modal-confirm-title')[0].innerHTML).toBe('title');
      expect($$('.infini-modal-confirm-content')[0].innerHTML).toBe('content');
      instance.update({
        title: 'new title',
        content: 'new content',
      });
      expect($$(`.infini-modal-confirm-${type}`)).toHaveLength(1);
      expect($$('.infini-modal-confirm-title')[0].innerHTML).toBe('new title');
      expect($$('.infini-modal-confirm-content')[0].innerHTML).toBe('new content');
      instance.destroy();
      jest.runAllTimers();
    });
    jest.useRealTimers();
  ...
  it('should be Modal.confirm without mask', () => {
    open({ mask: false });
    expect($$('.infini-modal-mask')).toHaveLength(0);
  });
