...
describe('TimeLine', () => {
  describe('renders items without passing any props correctly', () => {
    ...
    it('has 3 timeline item', () => {
      expect(wrapper.find('li.infini-timeline-item')).toHaveLength(3);
    });

    it('has only 1 timeline item is marked as the last item', () => {
      expect(wrapper.find('li.infini-timeline-item-last')).toHaveLength(1);
    });

    it('its last item is marked as the last item', () => {
      expect(
        wrapper
          .find('li.infini-timeline-item')
          .last()
          .hasClass('infini-timeline-item-last'),
      ).toBe(true);
    });
  });
  ...
  describe('renders pending item', () => {
    ...
    it('has one extra timeline item', () => {
      ...
      expect(wrapper.find('li.infini-timeline-item')).toHaveLength(4);
    });

    it('has extra pending timeline item', () => {
      const wrapper = wrapperFactory({ pending });
      expect(wrapper.find('li.infini-timeline-item-pending')).toHaveLength(1);
    });
    ...
    it("renders the pending timeline item as long as it receive a truthy prop value to 'pending'", () => {
      ...
      expect(wrapper.find('li.infini-timeline-item-pending')).toBeTruthy();
    });
    ...
    it('its last item is marked as the pending item', () => {
      ...
      expect(
        wrapper
          .find('li.infini-timeline-item')
          .last()
          .hasClass('infini-timeline-item-pending'),
      ).toBe(true);
    });

    it('its second to last item is marked as the last item', () => {
      const wrapper = wrapperFactory({ pending });
      const items = wrapper.find('li.infini-timeline-item');
      expect(items.at(items.length - 2).hasClass('infini-timeline-item-last')).toBe(true);
    });

    it('has the correct pending node', () => {
      const wrapper = wrapperFactory({ pending });
      expect(wrapper.find('li.infini-timeline-item-pending').contains(pending)).toBe(true);
    });

    it('has the correct pending dot node', () => {
      const wrapper = wrapperFactory({ pending, pendingDot });
      expect(wrapper.find('li.infini-timeline-item-pending').contains(pendingDot)).toBe(true);
    });

    it("has no pending dot if without passing a truthy 'pending' prop", () => {
      const wrapper = wrapperFactory({ pendingDot });
      expect(wrapper.find('li.infini-timeline-item-pending').contains(pendingDot)).toBe(false);
    });
  });
  ...
  describe('the item rendering sequence is controlled by reverse', () => {
    it('items is in order when prop reverse is false', () => {
      ...
      expect(wrapper.find('.infini-timeline-item-content').map(w => w.text())).toEqual([
        'foo',
        'bar',
        'baz',
      ]);
