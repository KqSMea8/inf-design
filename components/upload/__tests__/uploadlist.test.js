...
describe('Upload List', () => {
  ...
  // https://github.com/infini-design/infini-design/issues/4653
  it('should use file.thumbUrl for <img /> in priority', () => {
    ...
    fileList.forEach((file, i) => {
      const linkNode = wrapper.find('.infini-upload-list-item-thumbnail').at(i);
      const imgNode = wrapper.find('.infini-upload-list-item-thumbnail img').at(i);
      expect(linkNode.prop('href')).toBe(file.url);
      expect(imgNode.prop('src')).toBe(file.thumbUrl);
    });
  });

  // https://github.com/infini-design/infini-design/issues/7269
  it('should remove correct item when uid is 0', async () => {
    ...
    expect(wrapper.find('.infini-upload-list-item').length).toBe(2);
    wrapper
      .find('.infini-upload-list-item')
      .at(0)
      .find('.anticon-close')
      .simulate('click');
    await delay(400);
    wrapper.update();
    expect(wrapper.find('.infini-upload-list-item').hostNodes().length).toBe(1);
  });
  ...
  // https://github.com/infini-design/infini-design/issues/7762
  ...
  it('extname should work correctly when url not exists', () => {
    ...
    expect(wrapper.find('.infini-upload-list-item-thumbnail').length).toBe(2);
  });
  ...
  it('when picture-card is loading, icon should render correctly', () => {
    ...
    expect(wrapper.find('.infini-upload-list-item-uploading-text').length).toBe(1);
    expect(wrapper.find('.infini-upload-list-item-uploading-text').text()).toBe('uploading');
  });
  ...
  it('onPreview should be called, when url exists', () => {
    ...
    wrapper.find('.infini-upload-list-item-thumbnail').simulate('click');
    expect(onPreview).toHaveBeenCalled();
    wrapper.find('.infini-upload-list-item-name').simulate('click');
    expect(onPreview).toHaveBeenCalled();
