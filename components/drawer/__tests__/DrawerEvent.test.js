import React from 'react';
import { mount } from 'enzyme';
import Drawer from '..';
import Button from '../../button';

class DrawerEventTester extends React.Component {
  constructor(props) {
    super(props);
    this.state = { visible: false };
  }

  componentDidMount() {
    this.setState({ visible: true }); // eslint-disable-line react/no-did-mount-set-state
  }

  onClose = () => {
    this.setState({
      visible: false,
    });
  };

  open = () => {
    this.setState({
      visible: true,
    });
  };

  render() {
    const { visible } = this.state;
    return (
      <div>
        <Button onClick={this.open}>open</Button>
        <Drawer visible={visible} onClose={this.onClose} getContainer={false} {...this.props}>
          Here is content of Drawer
        </Drawer>
      </div>
    );
  }
}

describe('Drawer', () => {
  it('render correctly', () => {
    const wrapper = mount(<DrawerEventTester />);
    const body = wrapper.find('.infini-drawer-body').exists();

    expect(body).toBe(true);
    wrapper.find('button.infini-btn').simulate('click');

    const content = wrapper.find('.infini-drawer-body').getDOMNode().innerHTML;
    expect(content).toBe('Here is content of Drawer');

    expect(wrapper.render()).toMatchSnapshot();
  });

  it('mask trigger onClose', () => {
    const wrapper = mount(<DrawerEventTester />);

    wrapper.find('button.infini-btn').simulate('click');
    expect(wrapper.instance().state.visible).toBe(true);

    wrapper.find('.infini-drawer-mask').simulate('click');
    expect(wrapper.instance().state.visible).toBe(false);
  });

  it('close button trigger onClose', () => {
    const wrapper = mount(<DrawerEventTester />);

    wrapper.find('button.infini-btn').simulate('click');
    expect(wrapper.instance().state.visible).toBe(true);

    wrapper.find('.infini-drawer-close').simulate('click');
    expect(wrapper.instance().state.visible).toBe(false);
  });

  it('maskClosable no trigger onClose', () => {
    const wrapper = mount(<DrawerEventTester maskClosable={false} />);

    wrapper.find('button.infini-btn').simulate('click');
    expect(wrapper.instance().state.visible).toBe(true);

    wrapper.find('.infini-drawer-mask').simulate('click');
    expect(wrapper.instance().state.visible).toBe(true);
  });

  it('destroyOnClose is true onClose', () => {
    const wrapper = mount(<DrawerEventTester destroyOnClose />);
    wrapper.find('button.infini-btn').simulate('click');
    expect(wrapper.find('.infini-drawer-wrapper-body').exists()).toBe(true);

    wrapper.setState({
      visible: false,
    });
    wrapper.find('.infini-drawer-wrapper-body').simulate('transitionend');
    expect(wrapper.find('.infini-drawer-wrapper-body').exists()).toBe(false);
  });

  it('no mask and no closable', () => {
    const wrapper = mount(<DrawerEventTester destroyOnClose />);

    wrapper.find('button.infini-btn').simulate('click');
    expect(wrapper.instance().state.visible).toBe(true);

    wrapper.find('.infini-drawer-close').simulate('click');
    expect(wrapper.instance().state.visible).toBe(false);
  });
});
