...
describe('PageHeader', () => {
  it('pageHeader should not contain back it back', () => {
    ...
    expect(wrapper.find('.infini-page-header-back')).toHaveLength(0);
  });
  ...
  it('pageHeader should contain back it back', () => {
    ...
    expect(wrapper.find('.infini-page-header-back')).toHaveLength(1);
  });

  it('pageHeader onBack transfer', () => {
    const callback = jest.fn(() => true);
    const wrapper = mount(<PageHeader title="Page Title" onBack={callback} />);
    wrapper.find('div.infini-page-header-back-button').simulate('click');
